pipeline {
  ...
  stages {
    ...
    stage('send email') {
      steps {
        emailext(subject: 'Test Success: $PROJECT_NAME - Build # $BUILD_NUMBER - $BUILD_STATUS:  !', body: '$PROJECT_NAME - Build # $BUILD_NUMBER - $BUILD_STATUS:  Check console output at $BUILD_URL to view the results.  Selenium Script Execution was a success', attachLog: true, from: 'no-reply@ranvijay.in', replyTo: 'no-reply@ranvijay.in', to: 'hello@ranvijay.in')
      }
    }
  }
}